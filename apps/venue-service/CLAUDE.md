...
- **Publish RMQ event `venue.created` / `venue.updated`** sau create/update khi description non-empty, để search-ai-service seed tags từ description (xem `VenueEventsService`). Fire-and-forget — RMQ down ⇒ silent skip, không fail HTTP request. `update` chỉ emit khi `dto.description !== undefined` để tránh burn Groq quota cho PATCH chỉ đổi `opening_hours`/`media`/...
...
| GET | `/venues/top-rated?days=7&limit=6` | Public | Top-rated venues theo activity gần đây (Hot tuần này). Rank theo `rating_avg * LN(recent_review_count + 1.5)` từ cross-schema JOIN `interaction_schema.reviews`. `days` clamp [1,90], `limit` clamp [1,50] |
...
| `src/app/modules/venue/venue.service.ts` | Business logic — `findByOwner/findById` (kèm categories + `is_primary`), `findAll` etc. tự `attachCategories()` qua single SQL pass. `findTopRated(days, limit)` chạy raw SQL CTE đếm reviews trong window rồi rank theo `rating_avg * LN(recent_count + 1.5)`. Create/update fire-and-forget call `embeddingService.regenerateInBackground(id)` để sinh embedding **và** `events.emitDescribed(VENUE_CREATED|VENUE_UPDATED, ...)` để search-ai seed tags từ description. Update có `isSearchRelevantChange` guard: chỉ re-embed khi name/branch/desc/address đổi; emit RMQ chỉ khi `dto.description !== undefined`. |
...
| `src/app/modules/venue/venue-events.service.ts` | `VenueEventsService` — publisher chung cho VenueService + AdminVenueService, inject `EVENTS_SERVICE` ClientProxy (RmqModule.registerPublisher). Lazy connect ở `OnModuleInit`. `emitDescribed()` no-op khi description rỗng hoặc RMQ chưa connected — fire-and-forget, không block CRUD. |
...
|-------|-----------|-------|
| `venue.created` | **Publishes** | Sau `VenueService.create` / `createCommunity` / `AdminVenueService.create` — chỉ emit khi `description` non-empty. Payload: `{ venueId, name, branchName, description }` (`VenueDescribedEvent`). search-ai-service consume để seed tags từ description. |
| `venue.updated` | **Publishes** | Sau `VenueService.update` / `AdminVenueService.update` khi `dto.description !== undefined` (chủ ý sửa description). Payload giống `venue.created`. |

Publisher: `VenueEventsService` qua `RmqModule.registerPublisher({ name: 'EVENTS_SERVICE' })`. Fire-and-forget, không block HTTP response. RMQ down ⇒ silent skip (log warn).
...
PORT=3003
RABBITMQ_URL=amqp://localhost:5672    # optional — events skipped if absent
HUGGINGFACE_API_TOKEN=hf_...          # optional — embedding gen still works without
```

...
| File | Dùng cho |
|------|----------|
| `venue/dto/create-venue.dto.ts` | name, branch_name?, description?, address, city?, district?, latitude, longitude, total_capacity?, max_group_size?, is_group_friendly?, media?, opening_hours?, **category_ids?**, **primary_category_id?** |
...
| `src/app/modules/venue/venue.service.ts` | Business logic — findByOwner, findById (kèm categories), create, createCommunity, update, remove |

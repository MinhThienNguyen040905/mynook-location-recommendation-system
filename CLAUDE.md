...
Hệ thống **KHÔNG** dùng khái niệm "User" hay "Owner". Hệ thống dùng Entity **Account** với **AccountType** là `customer`, `business`, hoặc `admin`.

- Một `business` account quản lý nhiều venues (thông qua cột `business_id` trong bảng `venues`).
